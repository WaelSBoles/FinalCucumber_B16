package runner;
/*In Cucumber, the "Runner" class is responsible for executing the Cucumber scenarios and generating
the corresponding reports. The Runner class is typically written in Java and
serves as an entry point for executing the Cucumber tests.
 */
...
// runner class to execute all test cases
// we use these annotations to run the runner class(@RunWith(Cucumber.class) to get cucumber)
//@CucumberOptions( feature="path fo features in resources", glue ="package name" --> to stick them together ) to get the option in cucumber
